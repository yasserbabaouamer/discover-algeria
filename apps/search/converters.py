...
class SearchItemConverter:
    ...
    def convert_hotel_to_dto(self, hotel: Hotel) -> SearchItem:
        return SearchItem(
            hotel.id, 'hotels', hotel.cover_img.url, hotel.name,
    ...
    def convert_tour_to_dto(self, tour: PeriodicTour) -> SearchItem:
        return SearchItem(
            tour.id, 'tours', tour.cover_img.url, tour.title,
    ...
    def convert_agency_to_dto(self, agency: TouristicAgency) -> SearchItem:
        return SearchItem(
            agency.id, 'agencies', agency.cover_img.url, agency.name,
    ...
    def convert_city_to_dto(self, city: City) -> SearchItem:
        return SearchItem(
            city.id, 'destinations', city.cover_img.url, city.name, city.country.name, city.name_ratio
        )
